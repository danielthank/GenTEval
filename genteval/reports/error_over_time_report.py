"""Error over time report generator with MAPE and Cosine similarity analysis."""
...
class ErrorOverTimeReport(BaseReport):
    """Report generator for error over time evaluation with MAPE and Cosine similarity."""

    def __init__(self, compressors, root_dir, plot=True):
        """Initialize the error over time report generator."""
        super().__init__(compressors, root_dir)
        self.plot = plot
        self.time_series_metric = TimeSeriesComparisonMetric()

    def _calculate_error_over_time_fidelity(
    ...
    def generate(self, run_dirs) -> dict[str, Any]:
        """Generate error over time report with MAPE and Cosine similarity calculations."""
        results = {}
            ...
            for compressor in self.compressors:
                if compressor in {"original"}:
                    self.print_skip_message(
                        f"Compressor {compressor} is not supported for error over time evaluation, "
                ...
                if not original_results_file.exists():
                    self.print_skip_message(
                        f"Missing original error over time results for {app_name}_{service}_{fault}_{run}"
                    )
                    continue
                ...
                if not compressed_results_file.exists():
                    self.print_skip_message(
                        f"Missing compressed error over time results for {compressor} in {app_name}_{service}_{fault}_{run}"
                    )
                    continue
                ...
                for group_key in original_span_rate:
                    fidelity_result = self._calculate_error_over_time_fidelity(
